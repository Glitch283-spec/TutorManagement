...
import { Users, Clock, Loader2, CheckCircle2, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="mb-8">
        ...
        <p className="text-secondary-text">Monitor platform activities and learning requests.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="card-container flex items-center p-6 bg-primary/5 border-primary/20">
          ...
          <div>
            <p className="text-sm text-primary font-medium">Processing Requests</p>
        ...
        <div className="card-container flex items-center p-6 bg-success/5 border-success/20">
          ...
          <div>
            <p className="text-sm text-success font-medium">Assigned Requests</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link to="/review-request" className="text-primary hover:text-primary-hover text-sm font-medium flex items-center">
                  ...
                  {requests.slice(0, 5).map((request) => (
                    <tr key={request.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <p className="font-medium text-text">{request.profiles?.full_name}</p>
                        <p className="text-xs text-secondary-text">{request.profiles?.email}</p>
                      </td>
                      <td className="px-6 py-4 font-medium text-text">{request.student_name}</td>
                      <td className="px-6 py-4 text-secondary-text">{request.subject} - G{request.grade}</td>
                      <td className="px-6 py-4">
        ...
        <div className="space-y-4">
           ...
           {pendingCount === 0 ? (
              <div className="card-container text-center py-10">
                ...
                <p className="text-sm text-secondary-text">No pending requests to review.</p>
              </div>
           ) : (
             <div className="space-y-3">
               {requests.filter(r => r.status === 'Pending').slice(0, 4).map(request => (
                 <Link to={`/review-request`} key={request.id} className="block card-container p-4 hover:border-primary transition-colors hover:shadow-md cursor-pointer group">
                   <div className="flex justify-between items-start mb-2">
                     <h3 className="font-semibold text-text group-hover:text-primary transition-colors">{request.subject}</h3>
                     <span className="text-xs text-secondary-text">{format(new Date(request.created_at), 'MMM dd')}</span>
                   </div>
                   <p className="text-sm text-secondary-text mb-1">Student: {request.student_name}</p>
                   <p className="text-xs text-secondary-text truncate">{request.profiles?.full_name}</p>
                 </Link>
               ))}
             </div>
           )}
